...
class BoxView(QWidget):
    def __init__(self, parent=None):
        super(BoxView, self).__init__(parent)
    ...
    def setup_ui(self):
        ...
        form_layout.addRow("Marché:", self.marche_combo)
    ...
    def load_marches(self):
        marches = self.marche_service.getAll()
        self.marche_combo.clear()
        for marche in marches:
            self.marche_combo.addItem(marche.libelle, marche.id)

    def submit_form(self):
        try:
            marche_id = self.marche_combo.currentData()
            box = Box(
            ...
            if self.box_service.create_box(box):
                QMessageBox.information(self, "Succès", "Box enregistré avec succès!")
                self.clear_form()
            else:
                QMessageBox.critical(self, "Erreur", "Erreur lors de l'enregistrement du box")
        except ValueError as e:
            QMessageBox.warning(self, "Erreur de validation", "Veuillez vérifier les valeurs numériques")

    def clear_form(self):
        self.marche_combo.setCurrentIndex(0)
